...
import {
  ...
  CheckCircle, Wrench, Ban, Circle, Phone,
} from 'lucide-react';
import toast from 'react-hot-toast';
import {
  getComplexes,
  getCourts, createCourt, updateCourt, deleteCourt, getCourtTypes,
} from '@/api/courtApi';
import type {
  CourtComplex, Court, CourtFormData, CourtStatus,
...
function CourtModal({ mode, initial, complexId, courtTypes, onClose, onSave }: CourtModalProps) {
  ...
  const handleSubmit = async () => {
    if (!form.courtName.trim() || !form.courtCode.trim()) {
      toast.error('Tên sân và mã sân là bắt buộc!');
      return;
    }
    setSaving(true);
    try {
      await onSave(form, initial?.courtId);
      onClose();
    } finally {
      setSaving(false);
    }
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <Field label="Loại sân *">
                <select className="input-field" value={form.courtTypeId} onChange={(e) => set('courtTypeId', Number(e.target.value))}>
                  {courtTypes.map((t) => (
                    <option key={t.courtTypeId} value={t.courtTypeId}>{t.typeName}</option>
                  ))}
                </select>
              </Field>
              ...
              <div className="col-span-2">
                <Field label="Quy mô / Kích thước">
                  {courtTypes.find((t) => t.courtTypeId === form.courtTypeId)?.typeName?.toLowerCase().includes('bóng đá') ? (
                    <select
                      className="input-field"
                      value={form.courtSize || 'Sân 5 người'}
                      onChange={(e) => set('courtSize', e.target.value)}
                    >
                      <option value="Sân 5 người">Sân 5 người (mini)</option>
                      <option value="Sân 7 người">Sân 7 người</option>
                      <option value="Sân 11 người">Sân 11 người (tiêu chuẩn)</option>
                    </select>
                  ) : (
                    <input
                      className="input-field"
                      placeholder="VD: Tiêu chuẩn, Sân đơn, Sân đôi"
                      value={form.courtSize ?? ''}
                      onChange={(e) => set('courtSize', e.target.value)}
                    />
                  )}
                </Field>
              ...
              <div className="col-span-2">
                <Field label="URL hình ảnh đại diện (Ảnh bìa)" icon={<ImageIcon className="w-4 h-4" />}>
                  <input className="input-field" placeholder="https://..." value={form.imageUrl} onChange={(e) => set('imageUrl', e.target.value)} />
                </Field>
              </div>
              <div className="col-span-2">
                <Field label="Album ảnh phụ (Nhập mỗi link ảnh cách nhau bằng xuống dòng hoặc dấu phẩy)" icon={<ImageIcon className="w-4 h-4" />}>
                  <textarea
            ...
            <div className="space-y-5">
              ...
              <Field label="Giá cơ bản / giờ (VND) *" icon={<DollarSign className="w-4 h-4" />}>
                ...
                <p className="mt-1 text-xs text-slate-500">
                  Hiện tại: <span className="text-primary-400 font-medium">{fmtPrice(form.pricePerHour)}</span>/giờ
                </p>
              </Field>
              <div className="rounded-xl border border-surface-border bg-slate-800/30 p-4">
                <p className="text-xs text-slate-500 mb-3">💡 Bảng giá tham khảo theo khung giờ (dựa trên giá cơ bản)</p>
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-surface-border text-xs text-slate-500 font-medium">
                      <th className="text-left py-2">Khung giờ</th>
                      <th className="text-right py-2">Ngày thường</th>
                      <th className="text-right py-2">Cuối tuần</th>
                    </tr>
                  </thead>
...
function DeleteModal({ title, message, onClose, onConfirm }: DeleteModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={onClose} />
      <div className="relative bg-surface-card border border-surface-border rounded-2xl w-full max-w-sm shadow-2xl animate-fade-in p-6">
...
// ─────────────────────────────────────────────
// Main: Chi tiết Tổ hợp sân → Danh sách sân
// ─────────────────────────────────────────────
export default function ComplexDetailPage() {
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      try {
        const [complexes, co, ct] = await Promise.all([
          getComplexes(),
          getCourts({ complexId: cid }),
          getCourtTypes(),
        ]);
        setComplex(complexes.find((c) => c.complexId === cid) ?? null);
        setCourts(co);
        setCourtTypes(ct);
      } catch {
  ...
  const handleSaveCourt = useCallback(async (data: CourtFormData, courtId?: number) => {
    if (courtId) {
      ...
      const type = courtTypes.find((t) => t.courtTypeId === data.courtTypeId);
      setCourts((prev) => [
        ...prev,
        {
          ...created, ...data,
  ...
  return (
    <div className="p-6 space-y-6 min-h-full">
        ...
        <div className="flex items-start gap-5 p-5">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex flex-wrap gap-4 mt-1.5 text-xs text-slate-400">
              ...
              {complex.managerName && <span className="flex items-center gap-1"><Users className="w-3.5 h-3.5" />Quản lý: {complex.managerName}</span>}
            </div>
            {complex.description && (
              <p className="text-xs text-slate-500 mt-1.5 line-clamp-1">{complex.description}</p>
            )}
          </div>
          {/* Mini stats */}
      ...
      {/* ── Header of courts table ── */}
      <div className="flex items-center justify-between gap-4">
        <h2 className="text-base font-semibold text-white">
          Danh sách sân
          <span className="ml-2 text-sm text-slate-500 font-normal">({courts.length} sân)</span>
        </h2>
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="flex-1 min-w-52 flex items-center gap-2 bg-slate-800/60 border border-surface-border rounded-xl px-4 py-2.5">
          ...
          <input
            type="text"
            placeholder="Tìm theo tên, mã sân, vị trí..."
            className="flex-1 bg-transparent text-sm text-slate-300 placeholder:text-slate-600 outline-none"
            value={search}
            onChange={(e) => setSearch(e.target.value)}
          />
          {search && (
            <button onClick={() => setSearch('')} className="text-slate-500 hover:text-slate-300"><X className="w-3.5 h-3.5" /></button>
          )}
        </div>
                ...
                <tr key={court.courtId} className="hover:bg-slate-800/40 transition-colors group">
                  ...
                  <td className="py-3.5 pl-6 pr-4">
                    <div className="flex items-center gap-3">
                      <div 
                        ...
                        onClick={() => {
                          const allImages = [court.imageUrl, ...(court.imageUrls ?? [])].filter(Boolean) as string[];
                          if (allImages.length > 0) {
                            setViewerImages(allImages);
                            setViewerIndex(0);
                          } else {
                            toast.error("Sân này chưa có hình ảnh album.");
                          }
                        }}
                        title="Xem album ảnh"
                      >
                  ...
                  <td className="py-3.5 pl-4 pr-6">
                    <div className="flex items-center justify-center gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button
                        onClick={() => setModal({ kind: 'edit', court })}
                      ...
                      <button
                        onClick={() => setModal({ kind: 'delete', court })}
      ...
      {modal?.kind === 'delete' && (
        <DeleteModal
          title="Xóa sân"
          message={`Bạn có chắc muốn xóa sân "${modal.court.courtName}" (${modal.court.courtCode})? Hành động này không thể hoàn tác.`}
          onClose={() => setModal(null)}
          onConfirm={() => handleDeleteCourt(modal.court)}
        />
      )}
      ...
      {viewerImages && viewerImages.length > 0 && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-sm animate-fade-in">
          <button
            ...
            className="absolute top-4 right-4 p-2 rounded-full bg-slate-900/80 text-slate-400 hover:text-white border border-slate-800 transition-colors"
          >
            <X className="w-5 h-5" />
          </button>
