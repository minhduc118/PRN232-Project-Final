...
import { 
  Users, Package, Calendar, Activity, 
...
export default function StaffDashboardPage() {
  ...
  const fetchStats = async () => {
    try {
      ...
      const message = err instanceof Error ? err.message : 'Không thể tải thống kê bảng điều khiển.';
      toast.error(message);
    } finally {
      setLoading(false);
    }
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8 gap-4 animate-fade-in">
          <div>
            <h1 className="text-3xl font-extrabold text-white tracking-tight bg-gradient-to-r from-white via-slate-200 to-slate-400 bg-clip-text text-transparent">
              Trang quản trị nhân viên (Staff Home)
            </h1>
            <p className="text-slate-400 text-sm mt-1">
              Theo dõi trạng thái đặt sân, quản lý dụng cụ và thông tin tài khoản khách hàng thời gian thực.
            </p>
          </div>
          <button
            onClick={fetchStats}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8 animate-slide-up">
          ...
          <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-6 backdrop-blur-md shadow-xl flex items-center gap-5 hover:scale-[1.02] transition-transform duration-300">
            ...
            <div>
              <span className="text-xs text-slate-400 font-medium">Khách hàng thành viên</span>
          ...
          <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-6 backdrop-blur-md shadow-xl flex items-center gap-5 hover:scale-[1.02] transition-transform duration-300">
            ...
            <div>
              <span className="text-xs text-slate-400 font-medium">Tổng thiết bị dụng cụ</span>
              <strong className="block text-2xl font-black text-white mt-1 leading-none">
          ...
          <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-6 backdrop-blur-md shadow-xl flex items-center gap-5 hover:scale-[1.02] transition-transform duration-300">
            ...
            <div>
              <span className="text-xs text-slate-400 font-medium">Lượt đặt sân hôm nay</span>
              <strong className="block text-2xl font-black text-white mt-1 leading-none">
          ...
          <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-6 backdrop-blur-md shadow-xl flex items-center gap-5 hover:scale-[1.02] transition-transform duration-300">
            ...
            <div>
              <span className="text-xs text-slate-400 font-medium">Số sân hiện đang trống</span>
              <strong className="block text-2xl font-black text-white mt-1 leading-none">
          ...
          <div className="col-span-12 lg:col-span-8 bg-slate-900/60 border border-slate-800 rounded-2xl p-6 backdrop-blur-md shadow-xl">
            ...
            {recentBookings.length === 0 ? (
              <div className="py-12 text-center text-slate-500">
                ...
                <span className="text-sm">Hôm nay chưa có lượt đặt sân nào được đăng ký.</span>
              </div>
            ) : (
                ...
                <table className="w-full text-sm text-left text-slate-350">
                  <thead className="text-xs text-slate-400 uppercase bg-slate-900/50 border-b border-slate-850">
                    <tr>
                      ...
                      <th scope="col" className="px-4 py-3">Sân / Khung giờ</th>
                      <th scope="col" className="px-4 py-3">Tổng thanh toán</th>
                      <th scope="col" className="px-4 py-3">Trạng thái</th>
                    </tr>
                  </thead>
                  <tbody>
                    {recentBookings.map((bk) => (
                      <tr key={bk.bookingId} className="border-b border-slate-850 hover:bg-slate-900/20 transition-colors">
                        <td className="px-4 py-4 font-bold text-white">{bk.bookingCode}</td>
                        <td className="px-4 py-4 text-slate-200">{bk.customerName}</td>
                        <td className="px-4 py-4">
                          <span className="block text-slate-300 font-medium">{bk.courtName}</span>
                          <span className="block text-xs text-slate-500">{bk.slotName}</span>
                        </td>
                        <td className="px-4 py-4 font-semibold text-green-400">{formatPrice(bk.totalAmount)}</td>
                        <td className="px-4 py-4">
                          <span className={`badge px-2.5 py-0.5 text-[10px] uppercase font-bold rounded-full ${getStatusBadgeClass(bk.status)}`}>
          ...
          <div className="col-span-12 lg:col-span-4 space-y-6">
            ...
            <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-6 backdrop-blur-md shadow-xl">
              ...
              <div className="space-y-4 text-xs text-slate-400">
                <div className="p-3 bg-slate-800/40 rounded-xl border border-slate-850">
                  <strong className="block text-slate-200 mb-1">1. Quản lý kho dụng cụ</strong>
                  <span>Theo dõi tình trạng hỏng hóc, sửa đổi thiết bị và cập nhật số lượng tồn kho liên kết với dịch vụ cho thuê tương ứng.</span>
                </div>
                
                <div className="p-3 bg-slate-800/40 rounded-xl border border-slate-850">
                  <strong className="block text-slate-200 mb-1">2. Quản lý thông tin khách hàng</strong>
                  <span>Hỗ trợ tạo mới tài khoản cho khách đặt sân trực tiếp, điều chỉnh tích lũy loyalty points và cập nhật phân hạng thành viên.</span>
                </div>

                <div className="p-3 bg-slate-800/40 rounded-xl border border-slate-850">
                  <strong className="block text-slate-200 mb-1">3. Kiểm tra check-in sân</strong>
                  <span>Đối chiếu mã đặt sân (Booking Code) khi khách đến nhận sân đúng khung giờ hiển thị bên bảng điều khiển.</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-6 backdrop-blur-md shadow-xl">
              <div className="flex items-center justify-between mb-4">
                <h4 className="font-bold text-white text-xs uppercase tracking-wider text-slate-400">Hiệu năng hệ thống</h4>
