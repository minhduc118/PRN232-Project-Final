...
namespace SportCourtManagent_Server.DataAccess.Interfaces
{
    public interface IMaintenanceScheduleRepository
    {
        ...
        Task<(List<MaintenanceSchedule> Items, int TotalCount)> GetByComplexAsync(int complexId, MaintenanceStatus? status = null, int? assignedStaffId = null, int page = 1, int pageSize = 10);
