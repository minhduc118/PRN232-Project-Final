...
public class CourtService : ICourtService
{
  ...
  public async Task<PagedResult<CourtListDto>> SearchCourtsAsync(CourtSearchParams p)
  {
    ...
    var projected = query.Select(c => new CourtListDto
    {
      ...
      Location      = c.Location,
      ImageUrl      = c.CourtImages.OrderByDescending(ci => ci.IsPrimary).ThenBy(ci => ci.SortOrder).Select(ci => ci.ImageUrl).FirstOrDefault() ?? c.ImageUrl,
      Surface       = c.Surface,
      Capacity      = c.Capacity,
  ...
  public async Task<CourtDetailDto?> GetCourtDetailAsync(int courtId)
  {
    ...
    return new CourtDetailDto
    {
      ...
      ImageUrl    = court.ImageUrl ?? court.CourtImages.OrderByDescending(ci => ci.IsPrimary).ThenBy(ci => ci.SortOrder).Select(ci => ci.ImageUrl).FirstOrDefault(),
      OpenTime    = court.OpenTime,
      CloseTime   = court.CloseTime,
      Status      = court.Status.ToString(),
      CreatedAt   = court.CreatedAt,
      CourtType = new SportCourtManagerment.DTOs.CourtTypes.CourtTypeDto
      {
